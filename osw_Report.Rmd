...
This document functions as an all-inclusive working directory for synthesis and graphical analysis of the results from the offshore wind research of Morgan Browning, an ORISE Fellow at the U.S. Environmental Protection Agency's Office of Research and Development. 

This documents and its contents are not finalized nor are intended for publication. It is annotated primarily for ease of reproducability. Graphs are provided with many variations to meet criteria of different publication and presentation platforms.

# Setup

Sets global knit options and loads the setup and data import scripts for the project.
...
knitr::opts_chunk$set(echo = TRUE, include = TRUE, warning = FALSE, message = FALSE, tidy = TRUE, cache = FALSE)
```
```{r}
...
EIA's AEO 2019 provides the following values for the estimated levelized cost of electricity (capacity-weighted average) for new generation resources entering service in 2023 (2018 $/MWh). Offshore wind has the highest total LCOE by a large margin. The second most expensive technology is biomass. 
...
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(color > 0) cost40_facetcost_line_col
if(color > 0) cost40_facetcost_fill_col
```
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(bw > 0) cost40_facetcost_line_bw
if(bw > 0) cost40_facetcost_fill_bw
```

50% cost reduction

```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(color > 0) cost50_facetcost_line_col
if(color > 0) cost50_facetcost_fill_col
```
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(bw > 0) cost50_facetcost_line_bw
if(bw > 0) cost50_facetcost_fill_bw
```

60% cost reduction

```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(color > 0) cost60_facetcost_line_col
if(color > 0) cost60_facetcost_fill_col
```
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(bw > 0) cost60_facetcost_line_bw
if(bw > 0) cost60_facetcost_fill_bw
```

70% cost reduction

```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(color > 0) cost70_facetcost_line_col
if(color > 0) cost70_facetcost_fill_col
```
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(bw > 0) cost70_facetcost_line_bw
if(bw > 0) cost70_facetcost_fill_bw
```

80% cost reduction

```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(color > 0) cost80_facetcost_line_col
if(color > 0) cost80_facetcost_fill_col
```
```{r, out.width=c('50%', '50%'), fig.show='hold'}
if(bw > 0) cost80_facetcost_line_bw
if(bw > 0) cost80_facetcost_fill_bw
```

## Heatmaps

```{r}
if(color > 0) osw_grid_heatmap 
if(color > 0) wnd_grid_heatmap 
if(color > 0) sol_grid_heatmap 
if(color > 0) coal_grid_heatmap
if(color > 0) nga_grid_heatmap 
if(color > 0) nuk_grid_heatmap 
if(color > 0) hyd_grid_heatmap 
```
```{r}
